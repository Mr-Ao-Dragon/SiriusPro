...
public class AnnouncesImpl implements AnnouncesApi {
    ...
    @SneakyThrows
    @Override
    public Boolean deleteAnnouncesByGuildId(@NotNull String bot_id, @NotNull String guild_id, String message_id) {
        BotClient client = botPool.getBotById(bot_id);
        BotRequest botRequest = new BotRequest()
    ...
    @SneakyThrows
    @Override
    public Tuple<Announces, String> createGuildRecommend_Channels(@NotNull String bot_id, @NotNull String guild_id, @NotNull Integer announces_type, List<RecommendChannel> recommendChannels) {
        BotClient client = botPool.getBotById(bot_id);
